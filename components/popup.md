...
| 区分 | 項目 | 確認内容 | デフォルト値 |
|------|------|---------|------------|
| **必須** | トリガー画像ファイル名 | カラー/サイズ一覧の画像パス | — |
| **必須** | ポップアップ内の画像 | 各トリガーに対応する画像（複数可） | — |
| 任意 | ポップアップ内スライダー | あり or なし | あり（Slick使用）|
| 任意 | 閉じるボタン | ×ボタン or 背景クリック or 両方 | 両方 |
| 任意 | ポップアップの幅 | max-width | `600px` |
...
| 区分 | 項目 | 確認内容 | デフォルト値 |
|------|------|---------|------------|
| **必須** | トリガー要素 | クリックする要素（`<img>` or `<a>`）| — |
| **必須** | 表示する画像ファイル名 | `data-popup-img` 属性で指定 | — |
| 任意 | 閉じるボタン | ×ボタン + 背景クリック | 両方あり |
