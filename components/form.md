...
|------|------|---------|------------|
| **必須** | 入力項目 | 何を入力させるか | — |
| **必須** | ボタンテキスト | 送信ボタンのラベル | — |
| **必須** | 送信先URL | action属性のURL | — |
| 任意 | 各項目の必須/任意区分 | フォーム項目ごとの必須・任意 | 全項目「必須」 |
| 任意 | プライバシーポリシー | リンクの有無・URL | なし |
| 任意 | フォーム最大幅 | max-width | `600px` |
| 任意 | ボタンの角丸 | border-radius | `4px` |
...
| 区分 | 項目 | 確認内容 | デフォルト値 |
|------|------|---------|------------|
| **必須** | トリガーボタンテキスト | 折りたたみを開くボタンのラベル | — |
| **必須** | 展開後のフォーム内容 | F1と同様の入力項目確認 | — |
| 任意 | 初期状態 | 開いた状態 or 閉じた状態 | 閉じた状態 |
| 任意 | 送信先URL | action属性のURL | `#`（後で変更）|
| 任意 | プライバシーポリシー | リンクの有無・URL | なし |
...
| 区分 | 項目 | 確認内容 | デフォルト値 |
|------|------|---------|------------|
| **必須** | 閉じた状態の画像 | トリガーとなる画像ファイル名 | — |
| **必須** | 開いた状態の画像 | 展開後に表示する画像ファイル名 | — |
| 任意 | 初期状態 | 開いた状態 or 閉じた状態 | 閉じた状態 |
| 任意 | JS実装 | jQuery使用 or Vanilla JS | jQueryがある場合はjQuery版を使用 |
...
|------|------|---------|------------|
| **必須** | ステップ数 | 何ステップに分けるか | — |
| **必須** | 各ステップの入力項目 | ステップごとの入力内容 | — |
| **必須** | 送信先URL | 最終ステップのaction属性のURL | — |
| 任意 | プログレスバー | 進捗表示の有無 | あり |
| 任意 | プライバシーポリシー | リンクの有無・URL | なし |
