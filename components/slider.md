...
| 項目 | 確認内容 | 例 |
|------|---------|-----|
| スライドの中身 | 画像 / カード / ロゴ | 口コミカード画像 |
| 流れる速さ | `speed` の値（ms）| `5000`（遅め）〜 `2000`（速め）|
| 1画面の表示枚数 | `slidesToShow` | `3`（PC）/ `1.5`（SP）|
| 矢印・ドット | 表示 or 非表示 | 非表示 |
...
| 項目 | 確認内容 | 例 |
|------|---------|-----|
| SL2との組み合わせ | SL2の直下に配置するか | あり |
| 流れる速さ | SL2と同じ or 異なる速さ | 同じ（`5000`）|
